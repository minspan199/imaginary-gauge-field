...
totalMesh = (space + boundary+ spotSizeOuter)*(N+1)/meshSize;
...
for col = 1:N
    for row = 1:N
        nearFields{row, col} = Amp_sample(row, col) *...
            ...
            (ysNear - boundary*2 - (row-0.25) * space).^2) / (spotSizeOuter^2)))-...
            exp((-((xsNear - boundary*2 - (col-0.25) * space).^2 + ...
            (ysNear - boundary*2 - (row-0.25) * space).^2) / (spotSizeInner^2))))* ...
