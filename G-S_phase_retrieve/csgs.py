...
def rs(x, y, z, f, d, lam, res):
    ...
    for i in range(x.shape[0]):
        slm_p_phase[i, :] = 2.0 * numpy.pi / (lam * (f * 10.0 ** 3)) * (
                    x[i] * slm_xcoord[pup_coords] + y[i] * slm_ycoord[pup_coords]) + (numpy.pi * z[i]) / (
                                        lam * (f * 10.0 ** 3) ** 2) * (
    ...
    slm_total_field = numpy.sum(1.0 / (float(pup_coords[0].shape[0])) * numpy.exp(1j * (slm_p_phase + pists[:, None])),
                                axis=0)
    ...
    spot_fields = numpy.sum(
        1.0 / (float(pup_coords[0].shape[0])) * numpy.exp(1j * (slm_total_phase[None, :] - slm_p_phase)), axis=1)
    ...
    return out, [numpy.sum(ints), 1 - (numpy.max(ints) - numpy.min(ints)) / (numpy.max(ints) + numpy.min(ints)),
                 numpy.sqrt(numpy.var(ints)) / numpy.mean(ints), t]
...
def gs(x, y, z, f, d, lam, res, iters):
    ...
    for n in range(iters):
        # creation of the hologram, as superposition of all the phase patterns with random pistons
        slm_total_field = numpy.sum(
            1.0 / (float(pup_coords[0].shape[0])) * numpy.exp(1j * (slm_p_phase + pists[:, None])), axis=0)
        slm_total_phase = numpy.angle(slm_total_field)
...
def wgs(x, y, z, f, d, lam, res, iters):
    ...
    for n in range(iters):
        ...
        slm_total_field = numpy.sum(
            weights[:, None] / (float(pup_coords[0].shape[0])) * numpy.exp(1j * (slm_p_phase + pists[:, None])), axis=0)
        slm_total_phase = numpy.angle(slm_total_field)
...
def csgs(x, y, z, f, d, lam, res, iters, sub):
    ...
    for n in range(iters):
        ...
        slm_total_field = numpy.sum(1.0 / (float(pup_coords[0].shape[0])) * numpy.exp(
            1j * (slm_p_phase[:, coordslist_sparse] + pists[:, None])), axis=0)
        slm_total_phase = numpy.angle(slm_total_field)

        # Update of the phases at all spots locations. The intensities are evaluated too for performance
        # estimation of the algorithm
        spot_fields = numpy.sum(1.0 / (float(pup_coords[0].shape[0])) * numpy.exp(
            1j * (slm_total_phase[None, :] - slm_p_phase[:, coordslist_sparse])), axis=1)
        pists = numpy.angle(spot_fields)
        ints = numpy.abs(spot_fields) ** 2
